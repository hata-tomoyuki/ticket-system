...
MSBuild は各 `.csproj` をビルドするとき、`Directory.Build.props` を自動で読み込む。`import` を書く必要はない。

⚠️ 「親ディレクトリを遡って最初に見つけた 1 つだけを読む」という細かい挙動については、
このリポジトリでは 1 つしか置いていないので確かめていない。複数置く構成にするときは
[公式ドキュメント](https://learn.microsoft.com/en-us/visualstudio/msbuild/customize-by-directory)で確認すること。
...
🔬 セミコロンで終わっているのは C# 12 以降の書き方（`LangVersion` を 11 にすると `CS9058` で弾かれることを確認済み）。
...
C# が動くのは HTML を組み立てる一瞬だけで、返し終わったら止まる。WebSocket 接続も張られない。
（3 つのレンダーモードの違いは [rendermodes.md](rendermodes.md) にまとめてある）
...
🔬 ファイル名にハッシュらしき文字列が入っているのは実際に返ってきた HTML で確認した。

⚠️ その仕組み（`MapStaticAssets()` がビルド時にハッシュを計算し、`@Assets[...]` が対応表を引いている）は
筆者の理解であって、内部動作を確かめたわけではない。
確かなのは「書いた名前と返る名前が違う」という観測事実のほうで、
内容が変われば名前も変わるため、ブラウザキャッシュ由来の「直したのに反映されない」が起きにくくなる。
